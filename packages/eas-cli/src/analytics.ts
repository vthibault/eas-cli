...
export async function initAsync(): Promise<void> {
  ...
  if (process.env.DISABLE_EAS_ANALYTICS || amplitudeEnabled === false) {
    await UserSettings.setAsync('amplitudeEnabled', false);
    return;
  }
  if (!process.env.CI && amplitudeEnabled === null) {
    const amplitudeEnabled = await confirmAsync({
      message: 'Would you like to share anonymous usage data with the Expo Team?', // TODO better message
    });
    await UserSettings.setAsync('amplitudeEnabled', amplitudeEnabled);
    if (!amplitudeEnabled) {
      return;
    }
  }
